...
if (mainSource.includes('素材标记排到今日队列队首后处理') && mainSource.includes('素材缺口排到今日队列队首后处理') && mainSource.includes('图片任务排到今日队列队首后处理') && mainSource.includes('canRunMaterialActions && REVIEW_ACTIONS') && serverSource.includes('requireQueueHeadForCaseMaterial(req, asset.caseId') && serverSource.includes("requireQueueHeadForCaseMaterial(req, caze.id, '创建缺口图片任务'") && serverSource.includes("requireQueueHeadForCaseMaterial(req, task.caseId, '审核图片任务'") && smokeSource.includes('non-head asset patch was allowed') && smokeSource.includes('non-head image task was allowed to create') && smokeSource.includes('non-head image task was allowed to review') && smokeSource.includes('non-head image task was allowed to generate')) ok('素材标记、缺口建图和图片审核也受队首限制');
