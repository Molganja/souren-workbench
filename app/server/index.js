...
function queueStatusClass(status) {
  ...
  if (['已完成', 'approved', 'completed'].includes(status)) return 'ok';
  if (['素材阻塞', '异常', 'rejected', 'error', 'timeout', 'unknown'].includes(status)) return 'bad';
  if (['waiting_key', 'draft', 'waiting_edit', 'unavailable', 'waiting_chrome', '已取消'].includes(status)) return 'wait';
  if (['generating', 'review', 'handled'].includes(status)) return 'report';
  return 'bad';
}

function monitorActionStatusClass(kind) {
  if (kind === '偏冷补内容') return 'wait';
  if (kind === '增长承接') return 'report';
  return 'ready';
}
...
function dashboardPriorityActions(data = {}) {
  ...
  (data.viralAlerts || []).filter((alert) => !caseIsIntakeBlocked({ alert, case: alert.case })).forEach((alert) => {
    items.push({
      ...
      detail: `${alert.video?.title || alert.video?.url || '未命名作品'}｜播放 ${displayNumber(alert.snapshot?.plays)}｜评论 ${displayNumber(alert.snapshot?.comments)}`,
  ...
  (data.readyDelivery || []).filter((slot) => !caseIsIntakeBlocked({ slot, case: slot.case })).forEach((slot) => {
    items.push({
      ...
      kind: '微信交付',
      status: slot.status,
      statusClass: queueStatusClass(slot.status),
  ...
  (data.monitorActions || [])
    ...
    .forEach((item) => {
      items.push({
        ...
        kind: item.kind,
        status: item.title,
        statusClass: monitorActionStatusClass(item.kind),
  ...
  (data.clipTasks || []).filter((task) => !caseIsIntakeBlocked({ clipTask: task, case: task.case })).forEach((task) => {
    items.push({
      ...
      statusClass: queueStatusClass(task.status),
...
function dashboardQueueHead(data = {}) {
  return Array.isArray(data.priorityActions) ? data.priorityActions[0] || null : null;
}
...
app.post('/api/network/lan-access', (req, res) => {
  ...
  if (!['enable', 'disable'].includes(action)) return res.status(400).json({ error: '暂只支持开启或关闭局域网访问' });
  if (action === 'disable') {
    ...
    return res.json({
      ...
      message: '已关闭局域网访问配置。重启工作台后，只允许本机访问。'
    });
  }
...
function fixedEditRecipe(slot = {}, caze = {}, candidate = {}) {
  ...
  const recipes = {
    '素人种草': [
      '镜头1 0-3秒：放阶段标题或核心问题，画面用人物/过程/当前状态素材。',
      '镜头2 3-10秒：按“开始状态 -> 过程细节”顺序切画面，不打乱时间线。',
      '镜头3 10-25秒：按口播字幕解释当下感受、恢复阶段或项目细节。',
      '镜头4 25-35秒：回到当前状态或一个细节画面，保留评论引导。',
      '封面：人物或过程图 + 标题，标题不超过两行。'
    ],
    '日常养号': [
      '镜头1 0-2秒：生活画面先出现，标题只点出当天状态。',
      '镜头2 2-12秒：连续 3-5 个生活/城市/工作/穿搭素材，每个画面 2-3 秒。',
      '镜头3 12-25秒：字幕用碎碎念式记录，不额外加入医疗判断。',
      '镜头4 25-30秒：自然收尾，可以留一句轻互动。',
      '封面：使用第一张清晰生活图 + 短标题。'
    ],
    '爆款提权': [
      '镜头1 0-2秒：先放爆款结构里的反差、清单或情绪钩子。',
      '镜头2 2-8秒：给第一层理由或场景，字幕和画面同步。',
      '镜头3 8-22秒：按“三段式/清单式/反差式”固定结构推进，不临时加新段落。',
      '镜头4 22-30秒：结尾留互动问题或承接句。',
      '封面：标题优先，画面保持账号日常质感。'
    ]
  };
  ...
  return [
    '剪辑填空卡：',
    `1. 发给谁：${caze.weixinNick || '未命名兼职'}`,
    `2. 内容标题：${candidate.title || '按任务标题'}`,
    `3. 内容类型：${kind}`,
    '4. 素材：只用本次视频素材区的图片/视频，按页面顺序放。',
    '5. 完成：剪完或安排发布后，只在系统标记完成，不需要上传成片。',
    '',
    '只换这些空：标题、素材、封面图、收件账号。其他结构不重新想。',
    '',
    '固定剪辑配方：',
    '镜头固定模板：',
    ...lines,
    ...
    '3. 只替换素材和标题；剪辑只换这些空，不临时改结构、不临时加复杂特效。',
    '4. 口播/字幕优先使用本次锁定文案。',
    '5. 不重新创作脚本，不临时解释发布规则；发布说明已经写进交付文案。'
  ].join('\n');
}
